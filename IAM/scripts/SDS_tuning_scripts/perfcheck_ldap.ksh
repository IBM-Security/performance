...
# perfcheck_ldap.ksh
#
# Last Updated: 04/03/2012 Total rewrite
# Description:
...
#  All these things can cause the ldap search commands in this script to fail.
#
...
#	[-K <kdbfile>] [-h <host|IP>] [-p <port>][-o <output_file>][-v "<version>"]
#
#-h                    Print usage
#
...
GetVersionAndCommands()
{
	...
	if [[ -n "${SslTrue}" ]]; then
		if [[ -n "${AdminDn}" ]]; then
			...
			export CnConfigSearchCmd="${IDS_LDAP_SEARCH} -Z -P '${StashPw}' -K ${KeyDb} -D ${AdminDn} -w '${AdminPw}' -h ${Host} -p ${LdapPort} -b cn=Configuration -s sub objectclass=*"
			export CnConfigSearchCmdPwd="${IDS_LDAP_SEARCH} -Z -P ******** -K ${KeyDb} -D ${AdminDn} -w ******** -h ${Host} -p ${LdapPort} -b cn=Configuration -s sub objectclass=*"
		else
		...
		if [[ -n "${AdminDn}" ]]; then
			...
			export CnConfigSearchCmd="${IDS_LDAP_SEARCH} -D ${AdminDn} -w '${AdminPw}' -h ${Host} -p ${LdapPort} -b cn=configuration -s sub objectclass=*"
			export CnConfigSearchCmdPwd="${IDS_LDAP_SEARCH} -D ${AdminDn} -w ******** -h ${Host} -p ${LdapPort}  -b cn=configuration -s sub objectclass=*"
		else
...
printusage()
{
	...
	printf "\t[-K <kdbfile>][-H <host|IP>][-x][-p <port>]\n"
	printf "\t[-o <output_file>][-v <version>]\n"
	printf "\n"
...
while getopts "o:D:ZP:K:w:hxp:v:H:" opt
do
	 case $opt in
	 D )
			AdminDn="$OPTARG"
...
print "Version: ${LdapVersion}" >> ${OutputFile}

print "Beginning monitoring ..." >> ${OutputFile}
print "*******************************************" >> ${OutputFile}
print "" >> ${OutputFile}

print "Starting time: $(date)" >> ${OutputFile}
print "*******************************************" >> ${OutputFile}
...
# CN Monitor command
RunCmdsWithPassword "$CnMonitorCmdPwd" "$CnMonitorCmd"

# Check if a bind dn has been provided and try to do replication search and other stuff
if [ -n "$AdminDn" ]; then
	# search for existing connections
	RunCmdsWithPassword "$CnConnectionsCmdPwd" "$CnConnectionsCmd"
	# search for replication status attributes
	RunCmdsWithPassword "$ReplSearchCmdPwd" "$ReplSearchCmd"
	# Dump the config file
	RunCmdsWithPassword "$CnConfigSearchCmdPwd" "$CnConfigSearchCmd"
fi
