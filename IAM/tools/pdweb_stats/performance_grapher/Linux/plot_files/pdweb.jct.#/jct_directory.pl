...
#error determines is a file should be skipped or not

@logs = <$ARGV[1]/*>;	#all log files in this folder

LOGLOOP: foreach(@logs)
{
	...
	open (MYFILE, $_) || die("Cannot Open File");	#open input file
	@lines = <MYFILE>;
	if($lines[1] =~ /\[\/(.*)\]/)
	{
		...
		if($junct eq "")
		{
			$junct = "root_jct";		#handle blank jct name
		}
		else
		{
			$junct =~ s/\//\_slash\_/;	#replaces slashes to avoid invalid directory creation
		}
	}
	...
	foreach(@lines) 			#while file is non-empty
	{
		...
		else
		{
			...
			{
				print "\nWARNING: invalid log file: you have specified a $1 log file, need a jct log file\n\n";
				next LOGLOOP;
			}
		}
	}
	...
	system("mkdir $ARGV[0]\\pdweb_jct\\$junct");
...
set output \"$ARGV[0]/pdweb_jct/$filename1\"
...
set output \"$ARGV[0]/pdweb_jct/$filename4\"
