...
$prev_hits = 0;		#keeps track of previous number of hits

$file = "Time\tHits"; 					#organize gnudata columns
...
while (<MYFILE>) 			#while file is non-empty
{
 	...
 	if(/: (.*)/)	#$1 contains statistic
 	{
 		$data = $1;
		$temp = $data;
		$data = $data - $prev_hits;	#get difference
		$prev_hits = $temp;		#hold previous data
 	...
 	else		#no statistic (next block of stats)
 	{
 		...
 		{
 			print "\nERROR: invalid log file: you have specified a $1 log file, need a jmt log file\n\n";
 			exit 1;
 		}
 	}
}
...
#open (MYFILE, '>plot_files/pdweb.jmt/gnudata') || die("Cannot Open File");
#print MYFILE "$file";					#print output to file: gnudata
#close (MYFILE);

#plot output with gnuplot
#system("gnuplot/binary/gnuplot plot_files/pdweb.jmt/jmt_plot.dem");