...
#keep track of variable file in @ARGV[]
$fileNum = 1;
$inFile = "logs/vhj_1.log";
...
while( -e $inFile )		#while there is another vhj log file
{
	...
	system("if not exist current_output_graphs\\pdweb_vhj\\$junct mkdir current_output_graphs\\pdweb_vhj\\$junct");
...
set output \"current_output_graphs/pdweb_vhj/$filename1\"
...
set output \"current_output_graphs/pdweb_vhj/$filename4\"
plot 'plot_files/pdweb.vhj.#/gnudata' every ::2 using 1:5 index 0 with lines linewidth 2 linetype rgb \"blue\" title \"total\"";
	...
	#get next possible vhj log file location
	$fileNum++;
	$inFile = "logs/vhj_".$fileNum.".log";
}