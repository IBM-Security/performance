...
LOGLOOP: foreach(@logs)
{
	...
	foreach(@lines) 			#while file is non-empty
	{
		...
		else
		{
			...
			{
				print "\nWARNING: invalid log file: you have specified a $1 log file, need a vhj log file\n\n";
				next LOGLOOP;
			}
		}
	}
	...
	open (MYFILE, '>plot_files/pdweb.vhj.#/vhj_plot.dem') || die("Cannot Open File");
...
set xlabel \"Time\"  
set ylabel \"Reqs\"
set title \"Virtual Host Junction Performance: $junct\"
set key rmargin
