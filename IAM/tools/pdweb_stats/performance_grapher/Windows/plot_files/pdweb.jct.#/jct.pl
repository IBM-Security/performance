...
#keep track of variable file name
$inFile = "logs/jct_1.log";
$file_num = 1;

while( -e $inFile )			#while there is another vhj log file
{
	...
	open (MYFILE, $inFile) || die("Cannot Open File");	#open input file
	@lines = <MYFILE>;
	if($lines[1] =~ /\/(.*)\]/)
	{
		...
		if($junct eq "")
		{
			$junct = "root_jct";		#handle blank jct
		}
		else
		{
	...
	foreach(@lines) 			#while file is non-empty
	{
		...
		else
		{
			...
			{
				print "\nERROR: invalid log file: you have specified a $1 log file, need a jct log file\n\n";
				exit 1;
			}
		}
	}
	...
	close (MYFILE); 
	open (MYFILE, '>plot_files/pdweb.jct.#/gnudata') || die("Cannot Open File");
	print MYFILE "$file";					#print output to file: gnudata
	close (MYFILE);
	
	#create directories for each junction
	system("if not exist current_output_graphs\\pdweb_jct\\$junct mkdir current_output_graphs\\pdweb_jct\\$junct");

	#create gnuplot file to plot the data
	$filename1 = $junct . "/" . $junct . "_jct_plot_reqs.png";
	$filename2 = $junct . "/" . $junct . "_jct_plot_max_time.png";
	$filename3 = $junct . "/" . $junct . "_jct_plot_average_time.png";
	$filename4 = $junct . "/" . $junct . "_jct_plot_total_time.png";
	open (MYFILE, '>plot_files/pdweb.jct.#/jct_plot.dem') || die("Cannot Open File");
...
set xlabel \"Time\"  
set ylabel \"Reqs\"
set title \"Junction Performance: $junct\"
set key rmargin

#sets reqs output file and plots reqs data to it
set terminal png enhanced size 1280,1024
set output \"current_output_graphs/pdweb_jct/$filename1\"
...
set output \"current_output_graphs/pdweb_jct/$filename4\"
plot 'plot_files/pdweb.jct.#/gnudata' every ::2 using 1:5 index 0 with lines linewidth 2 linetype rgb \"blue\" title \"total\"";
	...
	system("gnuplot/binary/gnuplot plot_files/pdweb.jct.#/jct_plot.dem");
	
	#get next possible jct log file
	$fileNum++;
	$inFile = "logs/jct_".$fileNum.".log";
}