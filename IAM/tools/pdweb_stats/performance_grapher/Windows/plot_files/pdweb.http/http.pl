...
$file = "Time\treqs\tmax-worker\tavg-worker\ttotal-worker\tmax-webseal\tavg-webseal\ttotal-webseal";
...
close (MYFILE); 
open (MYFILE, '>plot_files/pdweb.http/gnudata') || die("Cannot Open File");
print MYFILE "$file";					#print output to file: gnudata
close (MYFILE);

#plot output with gnuplot
system("gnuplot/binary/gnuplot plot_files/pdweb.http/http_plot.dem");