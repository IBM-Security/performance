...
{ 	
 	...
 	else		#no statistic (next block of stats)
 	{
 		...
 		{
 			print "\nERROR: invalid log file: you have specified a $1 log file, need a certcallbackcache log file\n\n";
 			exit 1;
 		}
 	}
}
...
close (MYFILE); 
open (MYFILE, '>plot_files/pdweb.certcallbackcache/gnudata') || die("Cannot Open File");
print MYFILE "$file";					#print output to file: gnudata
close (MYFILE);

#plot output with gnuplot
system("gnuplot/binary/gnuplot plot_files/pdweb.certcallbackcache/certcallbackcache_plot.dem");